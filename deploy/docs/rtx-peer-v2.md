# O1/O2 peer-v2 deployment contract

This is the application peer-v2 transaction contract, selected through HomeLab
...
Each has separately configured `config`, `home`, `state` and `releases` roots,
...
Run focused regressions in a suitable isolated development environment (report
unavailable platform/runtime checks separately):
...
Resolve current host/path/port expectations through the
[HomeLab environment registry](https://github.com/Mortified2896/HomeLab/blob/main/docs/environments/current.json).
HomeLab owns installed manifests, units and the live identity gate.
This contract is a desired architecture, not evidence that either peer is accepted.

During initial migration only, `--legacy-supervisor-sha` binds the specifically
configured legacy O1 and its exact candidate services as supervisor for O2 updates.
...
The [initial rollout record](../../docs/evidence/peer-rollout-20260916.md) is
historical evidence, not a current release declaration. The `rtx` module/file
name is an existing application interface and does not select a development host.
