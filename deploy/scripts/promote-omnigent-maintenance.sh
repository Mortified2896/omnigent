#!/usr/bin/env bash
# Promote only the exact, fully accepted production artifact to maintenance.
set -euo pipefail
shopt -s nullglob
...
OMNIGENT_RELEASE_PREFLIGHT="${OMNIGENT_RELEASE_PREFLIGHT:-/home/hermes/workspace/repos/omnigent-production/deploy/scripts/omnigent_release_preflight.py}"
PROD_MARKER="$PROD_ROOT/maintenance-candidate.sha"
MAINT_MARKER="$MAINT_ROOT/DEPLOYED_SHA"
SERVER_UNIT=omnigent.service
...
create_maintenance_release() {
  ...
  "$OMNIGENT_RELEASE_PREFLIGHT" release "$source" "$sha" >/dev/null || refuse "production candidate failed release preflight"
  if [[ -e "$final" ]]; then
  ...
  local staging_root="$MAINT_ROOT/releases/.staging-$sha-$$" staging="$MAINT_ROOT/releases/.staging-$sha-$$/$sha"
...
PY
  ...
  "${SUDO[@]}" "$UV_PATH" venv --python "$PYTHON_PATH" "$staging/venv" || refuse "maintenance venv creation failed"
  "${SUDO[@]}" "$UV_PATH" pip install --python "$staging/venv/bin/python" "$staging/artifacts/$wheel_name[all]" \
    || refuse "maintenance wheel install failed"
  ...
  "$OMNIGENT_RELEASE_PREFLIGHT" release "$staging" "$sha" >/dev/null || refuse "installed maintenance release preflight failed"
...
restore_previous() {
  case "$PREVIOUS_KIND" in
    ...
    directory)
      ...
      ;;
    none) "${SUDO[@]}" rm -f "$MAINT_ROOT/venv" ;;
  esac
}

run_acceptance() {
  local release="$1" sha="$2" production_before="$3" body info installed_index hook production_after
  ...
  curl -fsS --max-time 10 "$MAINT_BASE_URL/" > "$body" || { rm -f "$body" "$info"; return 1; }
  installed_index=$("$release/venv/bin/python" -c 'from pathlib import Path; import omnigent; print(Path(omnigent.__file__).parent / "server/static/web-ui/index.html")') || return 1
  cmp -s "$installed_index" "$body" || { log "runtime root does not match the promoted installed SPA"; rm -f "$body" "$info"; return 1; }
  curl -fsS --max-time 10 "$MAINT_BASE_URL/v1/info" > "$info" || { rm -f "$body" "$info"; return 1; }
  python3 - "$info" "$release/PROVENANCE.txt" <<'PY' || { rm -f "$body" "$info"; return 1; }
import json, sys
...
PY
  ...
  capture_production_state "$production_after" || { rm -f "$body" "$info" "$production_after"; return 1; }
  cmp -s "$production_before" "$production_after" || { log "production service state changed during maintenance promotion"; rm -f "$body" "$info" "$production_after"; return 1; }
...
activate() {
  ...
  switch_venv "$release"
  if restart_units && run_acceptance "$release" "$sha" "$production_before"; then
    write_success_metadata "$release" "$sha"
    return 0
  fi
  log "full acceptance failed; restoring previous maintenance venv"
  restore_previous
  restart_units || fail "promotion and rollback restart both failed"
...
while (( $# )); do
  case "$1" in
    --rollback) mode=rollback; shift; (( $# )) || refuse "--rollback requires an exact SHA"; sha="$1"; shift ;;
    -h|--help) usage; exit 0 ;;
...
capture_production_state "$production_before" || refuse "could not capture production baseline"
if [[ "$mode" == promote ]]; then
  [[ -f "$PROD_MARKER" ]] || refuse "production has no fully accepted maintenance candidate marker"
  candidate=$(<"$PROD_MARKER")
  require_exact_sha "$candidate"
  [[ -z "$sha" ]] || { require_exact_sha "$sha"; [[ "$sha" == "$candidate" ]] || refuse "requested SHA is not the exact accepted production candidate"; }
  sha="$candidate"
  current_prod=$(readlink -f "$PROD_ROOT/current" 2>/dev/null || true)
  [[ "$current_prod" == "$PROD_ROOT/releases/$sha" ]] || refuse "production current symlink does not match its promotion marker"
  RELEASE_DIR=""
...
rm -f "$production_before"; trap - EXIT
