...
def metrics():
    lines = []
    for suffix, (wire, exporters) in otel.FILE_EXPORTERS.items():
...
def absent_failures():
    return {f"{prefix}_{suffix}": None for prefix in ("export_failed", "enqueue_failed") for suffix in otel.FILE_EXPORTERS}
...
class CounterApplicabilityTests(unittest.TestCase):
    def test_qualified_absence_is_explained_without_changing_nulls(self):
        ...
        for key, item in result.items():
            self.assertEqual(item["status"], "not_applicable" if key.startswith("enqueue") else "conditionally_absent")

    def test_unknown_or_different_profile_does_not_relax_checks(self):
        for profile in ({}, {"status": "verified"}, {"status": "unavailable", "profile": otel.FILE_ONLY_PROFILE}, {"status": "verified", "profile": "new-version"}):
    ...
    def test_missing_one_exporter_blocks_explanation_for_that_signal(self):
        body = "\n".join(line for line in metrics().splitlines() if "file/forensic_traces" not in line)
    ...
    def test_malformed_failure_is_not_an_absent_failure(self):
        for sample in ("NaN", "invalid", "-1", "0.5"):
            with self.subTest(sample=sample):
                ...
                self.assertEqual(otel.counter_evidence(body, absent_failures(), PROFILE)["export_failed_spans"]["status"], "unavailable")
    ...
    def test_observed_zero_and_failures_stay_observed(self):
        for value in (0, 3):
            counters = absent_failures() | {"export_failed_spans": value, "enqueue_failed_logs": value}
            result = otel.counter_evidence(metrics(), counters, PROFILE)
    ...
    def test_malformed_or_duplicate_sent_samples_are_not_activity_proof(self):
        for extra in ('otelcol_exporter_sent_spans{exporter="file/traces"} invalid\n', 'otelcol_exporter_sent_spans{exporter="file/traces"} 12\n'):
...
class RuntimeProfileTests(unittest.TestCase):
    def setUp(self):
        ...
        for name, value in (("OTEL_HOME", self.home), ("FILE_ONLY_CONFIG_SHA256", hashlib.sha256(self.config.read_bytes()).hexdigest())):
            self.stack.enter_context(patch.object(otel, name, value))
        self.stack.enter_context(patch.object(otel.sys, "platform", "darwin"))
        self.stack.enter_context(patch.object(otel, "_collector_process", return_value=("123", self.args)))
    ...
    def test_version_upgrade_is_not_silently_qualified(self):
        self.assertEqual(otel.collector_counter_profile("otelcol-contrib version 0.160.0", self.started + 1)["status"], "unavailable")
    ...
    def test_extra_cli_config_is_rejected(self):
        with patch.object(otel, "_collector_process", return_value=("123", [*self.args, "--set", "exporters=other"])):
    ...
    def test_process_started_after_scrape_is_rejected(self):
        self.assertEqual(otel.collector_counter_profile("otelcol-contrib version 0.159.0", self.started - 1)["reason"], "files_or_process_changed_since_launch")
    ...
    def test_process_change_during_probe_is_rejected(self):
        with patch.object(otel, "_collector_process", side_effect=[("123", self.args), ("124", self.args)]):
            self.assertEqual(self.profile()["status"], "unavailable")
    ...
    def test_probe_failure_does_not_leak_details(self):
        with patch.object(otel, "_collector_process", side_effect=subprocess.TimeoutExpired("private-canary", 3)):
            result = self.profile()
            self.assertEqual(result["status"], "unavailable")
            self.assertNotIn("private-canary", json.dumps(result))

    def test_no_mac_tools_on_other_platform(self):
        with patch.object(otel.sys, "platform", "linux"), patch.object(otel, "_probe_text", side_effect=AssertionError("no tools")):
...
class ProcessParsingTests(unittest.TestCase):
    def test_launchctl_argument_boundaries(self):
        ...
        with patch.object(otel, "_probe_text", return_value=text):
            self.assertEqual(otel._collector_process(), ("123", ["/path with spaces/collector", "--config", "/config path"]))

    def test_missing_or_ambiguous_launchctl_output_fails_closed(self):
        for text in ("", "pid = 1\n", "pid = 1\npid = 2\narguments = {\na\n}\n"):
            with self.subTest(text=text), patch.object(otel, "_probe_text", return_value=text), self.assertRaises(ValueError):
    ...
    def test_ps_start_is_parsed_as_utc(self):
        with patch.object(otel, "_probe_text", return_value="Thu Sep 10 13:00:00 2026"):
            self.assertEqual(otel._process_started("123"), dt.datetime(2026, 9, 10, 13, tzinfo=dt.timezone.utc).timestamp())

    def test_malformed_ps_output_fails_closed(self):
        with patch.object(otel, "_probe_text", return_value="unknown"), self.assertRaises(ValueError):
...
class ReportIntegrationTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.home / "state/retention-last-run.json").write_text(json.dumps({
            "run_at": dt.datetime.now(dt.timezone.utc).isoformat(),
            "archive_root": str(self.home / "data"), "dry_run": False,
            "converged": True, "errors": [],
            "max_bytes": otel.ARCHIVE_MAX_BYTES, "max_age_days": otel.ARCHIVE_MAX_AGE_DAYS,
            "forensic_max_bytes": otel.FORENSIC_MAX_BYTES, "forensic_max_age_days": otel.FORENSIC_MAX_AGE_DAYS,
        }))
        self.stack = contextlib.ExitStack()
        self.addCleanup(self.stack.close)
        ...
        self.stack.enter_context(patch.object(otel, "collector_counter_profile", return_value=PROFILE))
    ...
    def test_qualified_absence_is_not_incomplete(self):
        ...
        self.assertTrue(all(report["collector"][key] is None for key in report["unavailable_counters"]))
    ...
    def test_malformed_failure_stays_incomplete(self):
        with patch.object(otel, "prom", return_value=metrics() + "otelcol_exporter_send_failed_spans NaN\n"):
            self.assertEqual(otel.report()["state"], "INCOMPLETE")

    def test_missing_required_counter_stays_incomplete(self):
        body = "\n".join(line for line in metrics().splitlines() if not line.startswith("otelcol_receiver_refused_spans"))
        with patch.object(otel, "prom", return_value=body):
            self.assertEqual(otel.report()["state"], "INCOMPLETE")
    ...
    def test_check_passes_only_for_qualified_evidence(self):
        with patch.object(otel.sys, "argv", ["helper", "check", "--json"]), contextlib.redirect_stdout(io.StringIO()):
            otel.main()
        with patch.object(otel, "collector_counter_profile", return_value={}), patch.object(otel.sys, "argv", ["helper", "check", "--json"]), contextlib.redirect_stdout(io.StringIO()), self.assertRaises(SystemExit) as raised:
            otel.main()
