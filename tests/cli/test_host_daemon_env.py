...
@pytest.mark.parametrize("server_url", [None, _REMOTE_SERVER_URL])
@pytest.mark.parametrize("enabled", ["0", "1"])
def test_capture_survives_daemon_runner_and_spawned_codex_environment(
    monkeypatch: pytest.MonkeyPatch, tmp_path, server_url: str | None, enabled: str
) -> None:
    """Exercise the real filters and inspect the effective child environment."""
    import json
    import os
    import subprocess
    import sys
    ...
    monkeypatch.setenv("OMNIGENT_BENCHMARK_CAPTURE_DIR", capture_root)
    monkeypatch.setenv("OPERATOR_PRIVATE_TOKEN", "sentinel-never-forward")
    monkeypatch.setenv("OMNIGENT_UNRELATED_PRIVATE_TOKEN", "sentinel-never-forward")
    monkeypatch.delenv(RUNNER_ENV_PASSTHROUGH_ENV_VAR, raising=False)
    daemon = _build_host_daemon_env(server_url=server_url)
    runner = _build_runner_env(
        daemon,
        server_url=server_url or "http://127.0.0.1:8000",
        runner_id="capture-regression",
        binding_token="test-binding",
        workspace=str(tmp_path),
        parent_pid=os.getpid(),
    )
    with patch.dict(os.environ, runner, clear=True):
        harness = _clean_codex_env()
    ...
    for hop in (daemon, runner, harness, effective):
        assert hop["OMNIGENT_BENCHMARK_CAPTURE"] == enabled
        assert hop["OMNIGENT_BENCHMARK_CAPTURE_DIR"] == capture_root
        assert "OPERATOR_PRIVATE_TOKEN" not in hop
    # The local daemon owns operator config; its children must not inherit it.
    for hop in (runner, harness, effective):
        assert "OMNIGENT_UNRELATED_PRIVATE_TOKEN" not in hop
