...
PREFS = ProviderPreferences(
    True,
    ProviderSelection(selected_choice_ids=(OPENAI_LOW.choice_id, OPENAI_HIGH.choice_id)),
    ProviderSelection(selected_choice_ids=(GLM_HIGH.choice_id,)),
    OPENAI_HIGH.choice_id,
)
...
def test_transport_does_not_change_advisor_payload_or_candidate_ids():
    ...
    assert advisor_input("Task", effective_pool(PREFS, CATALOG)) == advisor_input(
        "Task", effective_pool(changed, CATALOG)
    )
...
def test_empty_enabled_pool_is_explicit_failure():
    with pytest.raises(ProviderPolicyError):
        effective_pool(
            PREFS.toggle_provider("openai", False).toggle_provider("glm", False), CATALOG
        )
...
@pytest.mark.parametrize(
    "routes",
    [(), (route(ready=False),), (route("direct", ready=False),), (route(choice=OPENAI_HIGH),)],
)
...
def test_gateway_fallback_must_be_same_plan_and_execution_contract(field):
    with pytest.raises(ProviderPolicyError):
        plan_transport(
            OPENAI_LOW, "omniroute_preferred", (route(), route("direct", **{field: "other"}))
        )
...
@pytest.mark.parametrize(
    "cause",
    ["timeout", "http_502", "quota_exhausted", "partial_stream", "unknown", "provider_401"],
)
...
def test_migration_deduplicates_lanes_without_erasing_old_settings():
    old = {
        "schema_version": 1,
        "enabled": True,
        "allowed_candidate_ids": ["gateway-old", "direct-old", "lost"],
        "advisor_candidate_id": "advisor-old",
        "human_probability_percent": 50,
    }
    before = json.dumps(old)
    migrated = migrate_v1(
        old, {"gateway-old": OPENAI_LOW, "direct-old": OPENAI_LOW, "advisor-old": GLM_HIGH}
    )
...
def test_migration_requires_explicit_policy_confirmation():
    old = {
        "schema_version": 1,
        "enabled": True,
        "allowed_candidate_ids": ["old"],
        "advisor_candidate_id": "old",
        "human_probability_percent": 50,
    }
