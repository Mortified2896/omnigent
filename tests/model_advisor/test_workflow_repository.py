...
class _Fixture:
    def setUp(self) -> None:
        ...
        self.judge = replace(
            self.human,
            candidate_id="advisor-high",
            model_id="fixture-advisor",
        ...
        self.round = freeze_round(
            ...
            human_candidate_id="small-low",
        )
        self.advice = json.dumps(
            {"candidate_id": "glm-high", "rationale": "The task benefits from more reasoning."}
        )
...
class WorkflowTests(_Fixture, unittest.TestCase):
    ...
    def test_extra_settings_fields_are_rejected(self) -> None:
        with self.assertRaises(AdvisorContractError):
            AdvisorPreferences.from_payload(
                {**self.prefs.to_payload(), "api_key": "not-a-real-key"}
            )
    ...
    def test_strict_settings_types(self) -> None:
        cases = (
            ...
            {"allowed_candidate_ids": "small-low"},
        )
        for patch in cases:
    ...
    def test_missing_saved_choice_does_not_silently_fallback(self) -> None:
        with self.assertRaises(AdvisorContractError):
            freeze_round(
                ...
                task="Task",
                preferences=self.prefs,
                catalog=PoolSnapshot("v2", (self.human, self.judge)),
                human_candidate_id="small-low",
            )
    ...
    def test_disabled_preferences_cannot_start_round(self) -> None:
        with self.assertRaises(AdvisorContractError):
            freeze_round(
                ...
                settings_revision="s",
                task="Task",
                preferences=replace(self.prefs, enabled=False),
                catalog=self.catalog,
                human_candidate_id="small-low",
            )
    ...
    def test_stable_candidate_id_ignores_display_copy(self) -> None:
        self.assertEqual(
            stable_candidate_id(self.human),
            stable_candidate_id(
                replace(self.human, capability_summary="New wording", candidate_id="other-handle")
            ),
        )

    def test_candidate_id_binds_lane_and_effort(self) -> None:
        for other in (
            replace(self.human, lane_id="other"),
            replace(self.human, reasoning_effort="high"),
            replace(self.human, connection_id="another-account"),
        ):
    ...
    def test_fingerprint_binds_prompt_settings_and_probability(self) -> None:
        for other in (
            replace(self.round, task="Edited"),
            replace(self.round, settings_revision="s2"),
            replace(self.round, human_probability_percent=60),
        ):
    ...
    def test_fifty_fifty_uses_existing_core(self) -> None:
        for bit, arm in ((0, "human"), (1, "advisor")):
            ...
            def draw(bound: int, *, bit: int = bit, bounds: list[int] = bounds) -> int:
                bounds.append(bound)
                return bit
    ...
    def test_weighted_probability_records_real_propensity(self) -> None:
        ...
        self.assertEqual(
            (human.original_assignment.arm, human.original_assignment.probability_numerator),
            ("human", 30),
        )
        self.assertEqual(
            (advisor.original_assignment.arm, advisor.original_assignment.probability_numerator),
            ("advisor", 70),
        )
    ...
    def test_extreme_probabilities_do_not_randomize(self) -> None:
        for pct, arm in ((0, "advisor"), (100, "human")):
            review = prepare_review(
                replace(self.round, human_probability_percent=pct),
                self.advice,
                randbelow=never_draw,
            )
            self.assertEqual(review.original_assignment.arm, arm)
    ...
    def test_bad_rng_does_not_make_assignment(self) -> None:
        with self.assertRaises(AdvisorContractError):
            prepare_review(
                replace(self.round, human_probability_percent=30),
                self.advice,
                randbelow=lambda _: 100,
            )
    ...
    def test_outside_advice_rejected(self) -> None:
        for key in ("invented", "advisor-high"):
            with self.assertRaises(AdvisorContractError):
                prepare_review(
                    self.round,
                    json.dumps({"candidate_id": key, "rationale": "x"}),
                    randbelow=never_draw,
                )
    ...
    def test_override_retains_draw_and_original_picks(self) -> None:
        review = prepare_review(self.round, self.advice, randbelow=lambda _: 1)
        override = confirm_review(
            self.round,
            review,
            self.catalog,
            override_candidate_id="small-low",
            reason="Prefer lower usage",
        )
    ...
    def test_override_requires_reason_and_allowed_candidate(self) -> None:
        ...
        for key, reason in (("small-low", None), ("advisor-high", "manual")):
            with self.assertRaises(AdvisorContractError):
                confirm_review(
                    self.round, review, self.catalog, override_candidate_id=key, reason=reason
                )
...
class RepositoryTests(_Fixture, unittest.TestCase):
    ...
    def test_stale_save_rejected(self) -> None:
        ...
        second = self.repo.save_preferences(
            "owner",
            "host",
            replace(self.prefs, human_probability_percent=30),
    ...
    def test_new_defaults_do_not_change_existing_round(self) -> None:
        ...
        self.repo.save_preferences(
            "owner", "host", replace(self.prefs, human_probability_percent=0), expected_version=1
        )
    ...
    def test_assignment_survives_restart_without_draw(self) -> None:
        self.repo.reserve_round(self.round)
        first = self.repo.finish_advice(
            "owner", "host", "round-1", self.advice, randbelow=lambda _: 1
        )
        self.engine.dispose()
        restarted = create_engine(self.url)
        replay = AdvisorRepository(restarted).finish_advice(
            "owner", "host", "round-1", self.advice, randbelow=never_draw
        )
        self.assertFalse(replay.acquired)
    ...
    def test_changed_advice_cannot_replace_committed_result(self) -> None:
        ...
        with self.assertRaises(AdvisorConflict):
            self.repo.finish_advice(
                "owner",
                "host",
                "round-1",
                '{"candidate_id":"small-low","rationale":"other"}',
                randbelow=never_draw,
            )
    ...
    def test_confirmation_claims_execution_only_once(self) -> None:
        self.repo.reserve_round(self.round)
        review = self.repo.finish_advice(
            "owner", "host", "round-1", self.advice, randbelow=lambda _: 1
        )
        kwargs = {"expected_version": review.record.version, "live_catalog": self.catalog}
    ...
    def test_changed_confirmation_is_conflict(self) -> None:
        ...
        self.repo.confirm(
            ...
            "host",
            "round-1",
            expected_version=review.record.version,
    ...
    def test_unavailable_assigned_model_does_not_claim_execution(self) -> None:
        ...
        with self.assertRaises(AdvisorContractError):
            self.repo.confirm(
                ...
                "round-1",
                expected_version=review.record.version,
                live_catalog=PoolSnapshot("new", (self.human,)),
            )
    ...
    def test_other_owner_cannot_read_or_confirm_round(self) -> None:
        ...
        with self.assertRaises(AdvisorConflict):
            self.repo.confirm(
                "other", "host", "round-1", expected_version=1, live_catalog=self.catalog
            )
    ...
    def test_concurrent_finish_has_one_committed_draw(self) -> None:
        ...
        def work() -> None:
            try:
                ...
                results.append(
                    self.repo.finish_advice(
                        "owner", "host", "round-1", self.advice, randbelow=draw
                    ).acquired
                )
    ...
    def test_bind_dispatch_records_the_exact_session_once(self) -> None:
        self.repo.reserve_round(self.round)
        advice = self.repo.finish_advice(
            "owner", "host", "round-1", self.advice, randbelow=lambda _: 1
        )
        self.repo.confirm(
            "owner",
            "host",
            "round-1",
            expected_version=advice.record.version,
            live_catalog=self.catalog,
        )
        bound = self.repo.bind_dispatch("owner", "host", "round-1", session_id="conv_123")
        self.assertTrue(bound.acquired)
        self.assertEqual(bound.record.state, "dispatch_bound")
        self.assertEqual(bound.record.payload["execution_session_id"], "conv_123")
        replay = self.repo.bind_dispatch("owner", "host", "round-1", session_id="conv_123")
        self.assertFalse(replay.acquired)
        with self.assertRaises(AdvisorConflict):
            self.repo.bind_dispatch("owner", "host", "round-1", session_id="conv_other")

    def test_bind_dispatch_requires_a_claimed_round(self) -> None:
        self.repo.reserve_round(self.round)
        with self.assertRaises(AdvisorConflict):
            self.repo.bind_dispatch("owner", "host", "round-1", session_id="conv_123")

    def test_crash_after_claim_stays_uncertain_not_retried(self) -> None:
        """dispatch_claimed without a binding is inspectable, never replayable."""
        self.repo.reserve_round(self.round)
        advice = self.repo.finish_advice(
            "owner", "host", "round-1", self.advice, randbelow=lambda _: 1
        )
        ...
        record = self.repo.load_round("owner", "host", "round-1")
        self.assertEqual(record.state, "dispatch_claimed")
        self.assertIsNone(record.payload.get("execution_session_id"))
        # Even a fresh confirm with the same body returns the claimed record.
        replay = self.repo.confirm(
            "owner",
            "host",
            "round-1",
    ...
    def test_mark_round_failed_blocks_only_pending_rounds(self) -> None:
        self.repo.reserve_round(self.round)
        blocked = self.repo.mark_round_failed("owner", "host", "round-1", reason="advisor timeout")
        self.assertEqual(blocked.state, "blocked")
        self.assertEqual(blocked.payload["failure_reason"], "advisor timeout")
        with self.assertRaises(AdvisorConflict):
            self.repo.mark_round_failed("owner", "host", "round-1", reason="again")
        with self.assertRaises(AdvisorConflict):
            self.repo.mark_round_failed("owner", "host", "missing-round", reason="x")

    def test_overhead_recording_preserves_state_and_drops_from_terminal(self) -> None:
        self.repo.reserve_round(self.round)
        overhead = {"latency_ms": 500, "input_tokens": 10}
        recorded = self.repo.record_advisor_overhead("owner", "host", "round-1", overhead=overhead)
        self.assertEqual(recorded.state, "advisor_pending")
        self.assertEqual(recorded.payload["advisor_overhead"], overhead)
        self.repo.mark_round_failed("owner", "host", "round-1", reason="timeout")
        self.assertIsNone(
            self.repo.record_advisor_overhead("owner", "host", "round-1", overhead=overhead)
        )
