...
# e2e set), so their profiles are built directly here. Only the two
# OMNIGENT_CREDENTIAL vendors are shipped as official: the bench can mint
# their gateway credential, so the native-tui driver can actually run them
# on a host with the vendor CLI logged in. OWN_AUTH natives (cursor-native,
# kiro-native, ...) need a vendor login the bench cannot provision, so they
# are left to a --harness <ref> opt-in rather than the default set.
#
