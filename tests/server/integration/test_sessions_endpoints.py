...
async def test_external_subagent_start_mints_child_session(
    ...
    assert (
        child["labels"]["omnigent.harnesses.claude_native.main.description"]
        == "Trace the auth flow"
    )
...
async def test_external_subagent_start_adopts_unlabeled_title_collision(
    ...
    assert (
        child["labels"]["omnigent.harnesses.claude_native.main.subagent_id"] == "a5c7effac5a9a35ab"
    )
    assert (
        child["labels"]["omnigent.harnesses.claude_native.main.description"]
        == "Trace the auth flow"
...
async def test_post_external_codex_collaboration_mode_change_persists_label(
    ...
    snapshot = (await client.get(f"/v1/sessions/{session['id']}")).json()
    assert snapshot["labels"]["omnigent.harnesses.codex_native.main.collaboration_mode"] == "plan"
...
async def test_post_external_permission_mode_change_persists_label_and_publishes(
    ...
    snapshot = (await client.get(f"/v1/sessions/{session['id']}")).json()
    assert snapshot["labels"]["omnigent.harnesses.claude_native.main.permission_mode"] == "auto"
...
async def test_patch_collaboration_mode_persists_label_and_forwards_event(
    ...
    assert resp.status_code == 200, resp.text
    assert (
        resp.json()["labels"]["omnigent.harnesses.codex_native.main.collaboration_mode"] == "plan"
    )
...
async def test_patch_collaboration_mode_requires_live_runner_before_persisting(
    ...
    assert "omnigent.harnesses.codex_native.main.collaboration_mode" not in snapshot["labels"]
    assert published == []
...
async def test_patch_approval_mode_forwards_and_persists_label(
    ...
    assert resp.status_code == 200, resp.text
    assert (
        resp.json()["labels"]["omnigent.harnesses.codex_native.main.approval_mode"]
        == "full-access"
    )
...
async def test_patch_approval_mode_requires_live_runner_before_persisting(
    ...
    assert "omnigent.harnesses.codex_native.main.approval_mode" not in snapshot["labels"]
    assert published == []
...
async def test_post_external_codex_approval_mode_change_sets_label_and_publishes(
    ...
    snapshot = (await client.get(f"/v1/sessions/{session['id']}")).json()
    assert (
        snapshot["labels"]["omnigent.harnesses.codex_native.main.approval_mode"] == "full-access"
    )
...
async def test_patch_permission_mode_persists_label_and_forwards_event(
    ...
    assert resp.status_code == 200, resp.text
    assert resp.json()["labels"]["omnigent.harnesses.claude_native.main.permission_mode"] == "auto"
...
async def test_patch_permission_mode_rewrites_launch_arg_and_keeps_other_args(
    ...
    assert (
        resp.json()["labels"]["omnigent.harnesses.claude_native.main.permission_mode"]
        == "acceptEdits"
    )
...
async def test_patch_permission_mode_requires_live_runner_before_persisting(
    ...
    assert "omnigent.harnesses.claude_native.main.permission_mode" not in snapshot["labels"]
...
async def test_external_codex_subagent_start_mints_child_session(
    ...
    labels = child["labels"]
    assert (
        labels["omnigent.harnesses.codex_native.main.subagent_thread_id"] == "thread_child_alpha"
    )
    assert labels["omnigent.harnesses.codex_native.main.parent_thread_id"] == "thread_parent"
    assert labels["omnigent.harnesses.codex_native.main.collab_tool_call_id"] == "collab_123"
    assert labels["omnigent.harnesses.codex_native.main.agent_nickname"] == "auth-auditor"
    assert labels["omnigent.harnesses.codex_native.main.agent_role"] == "reviewer"
...
async def test_external_antigravity_subagent_start_mints_child_session(
    ...
    assert (
        labels["omnigent.harnesses.antigravity_native.main.subagent_cascade_id"]
        == "1eca7625-be65-4092-8718-273c1bc3436b"
    )
    assert (
        labels["omnigent.harnesses.antigravity_native.main.agent_role"]
        == "App Router Code Reviewer"
    )
    assert labels["omnigent.harnesses.antigravity_native.main.agent_type"] == "research"
    # Links the child back to the tool card that spawned it.
    assert (
        labels["omnigent.harnesses.antigravity_native.main.tool_call_id"] == "agy_call_efb134b2_36"
    )
...
async def test_external_antigravity_subagent_start_title_survives_a_colon_in_the_role(
    ...
    assert (
        child["labels"]["omnigent.harnesses.antigravity_native.main.agent_role"]
        == "Review: routing and auth"
    )
...
async def test_external_codex_subagent_start_adopts_unlabeled_title_collision(
    ...
    # normal label lookup.
    assert (
        children[0]["labels"]["omnigent.harnesses.codex_native.main.subagent_thread_id"]
        == "thread_child_gamma"
    )
