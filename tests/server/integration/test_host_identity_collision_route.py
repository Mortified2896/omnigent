...
async def test_tunnel_rejects_live_owner_name_collision(db_uri: str) -> None:
    ...
    await communicator.send_input({"type": "websocket.receive", "text": encode_host_frame(hello)})
    reported = await communicator.receive_output(timeout=1.0)
    assert reported["type"] == "websocket.send"
    error = decode_host_frame(reported["text"])
