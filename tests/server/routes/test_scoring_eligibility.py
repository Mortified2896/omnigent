...
def seed_answer(store, conversation_id):
    store.append(conversation_id, [NewConversationItem(
        type="message", response_id="answer",
        data=MessageData(role="assistant", agent="test", content=[{"type": "output_text", "text": "Done"}]),
    )])
...
def client_for(store, auth=None, permissions=None):
    ...
    router = APIRouter()
    register_feedback_routes(router, conversation_store=store, auth_provider=auth, permission_store=permissions)
...
def test_exclusion_before_outcome_reload_and_raw_review_preservation(db_uri):
    ...
    with client_for(store) as client:
        ...
        response = client.put(f"{url}/scoring-eligibility/answer", json={
            "score_eligible": False, "exclusion_reason": "test_fixture",
        })
    ...
    with client_for(reopened) as client:
        ...
        assert policy["responses"]["answer"] == {"score_eligible": False, "exclusion_reason": "test_fixture"}
        assert client.put(f"{url}/scoring-eligibility/answer", json={"score_eligible": True}).status_code == 200
        ...
        assert client.put(f"{url}/scoring-eligibility/answer", json={"score_eligible": False}).status_code == 200
        assert client.put(f"{url}/task-outcomes/answer", json={"outcome": "failed"}).status_code == 200
        assert client.get(f"{url}/scored-outcomes").json() == []


@pytest.mark.parametrize("body", [
    {}, {"score_eligible": "false"}, {"score_eligible": 0},
...
def test_test_session_exclusion_applies_even_to_legacy_outcomes(db_uri):
    ...
    url = f"/v1/sessions/{conv.id}"
    with client_for(store) as client:
        assert client.put(f"{url}/task-outcomes/answer", json={"outcome": "success"}).status_code == 200
        ...
        assert client.put(f"{url}/scoring-eligibility/answer", json={"score_eligible": True}).status_code == 422
        assert client.put(f"{url}/scoring-eligibility/missing", json={"score_eligible": False}).status_code == 404
...
def test_scoring_routes_are_caller_scoped_and_permission_gated(db_uri):
    ...
    with client_for(store, Caller(), permissions) as client:
        for suffix in ("scoring-policy", "scored-outcomes"):
            ...
            assert client.get(f"{url}/{suffix}", headers={"x-test-user": "stranger"}).status_code == 404
        payload = {"score_eligible": False}
        assert client.put(f"{url}/scoring-eligibility/answer", json=payload, headers={"x-test-user": "reader"}).status_code == 403
        assert client.put(f"{url}/scoring-eligibility/answer", json=payload, headers={"x-test-user": "alice"}).status_code == 200
        assert client.get(f"{url}/scoring-policy", headers={"x-test-user": "bob"}).json()["responses"] == {}
        assert client.get(f"{url}/scoring-policy", headers={"x-test-user": "alice"}).json()["responses"]["answer"]["score_eligible"] is False
        assert client.put(f"/v1/sessions/{other.id}/scoring-eligibility/answer", json=payload, headers={"x-test-user": "alice"}).status_code == 404
