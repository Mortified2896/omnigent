...
async def test_fork_session_run_config_overrides_pass_through() -> None:
    ...
    assert (
        "omnigent.harnesses.claude_native.main.permission_mode" in fork_call["dropped_label_keys"]
    )
...
async def test_fork_switch_drops_claude_permission_mode_label() -> None:
    ...
    assert "omnigent.harnesses.claude_native.main.permission_mode" in dropped, (
...
async def test_fork_codex_bypass_stamps_label_on_codex_target(
    ...
    assert extra == {"omnigent.harnesses.codex_native.main.bypass_sandbox": "1"}, (
