...
def test_fork_conversation_drops_instance_scoped_labels(
    ...
    conversation_store.set_labels(
        source.id,
        {
            "omnigent.harnesses.claude_native.main.bridge_id": source.id,
            "omnigent.harnesses.codex_native.main.bridge_id": source.id,
...
def test_fork_extra_labels_rearm_bypass_over_the_always_drop(
    ...
    conversation_store.set_labels(
        source.id, {"omnigent.harnesses.codex_native.main.bypass_sandbox": "1"}
    )

    # Same-agent fork WITHOUT the opt-in: the source's label is dropped.
    plain = conversation_store.fork_conversation(source.id)
    assert "omnigent.harnesses.codex_native.main.bypass_sandbox" not in plain.labels

    # WITH the opt-in via extra_labels: armed on the fork despite the drop.
    armed = conversation_store.fork_conversation(
        source.id,
        extra_labels={"omnigent.harnesses.codex_native.main.bypass_sandbox": "1"},
    )
    assert armed.labels.get("omnigent.harnesses.codex_native.main.bypass_sandbox") == "1", (
...
def test_switch_conversation_agent_cross_family_resets_and_relabels(
    ...
    assert "omnigent.harnesses.codex_native.main.bypass_sandbox" not in updated.labels, (
