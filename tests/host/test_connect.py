...
async def test_o3_catalog_preserves_explicit_lanes_and_native_efforts(monkeypatch) -> None:
    from omnigent.harnesses.codex_native import app_server
    from omnigent.host.connect import _with_model_configuration_source
    ...
    assert sum(row.get("isDefault", False) for row in result.models) == 1
    assert all(
        row["supportedReasoningEfforts"] == [{"reasoningEffort": "ultra"}] for row in result.models
    )
    monkeypatch.setattr(
        "omnigent.host.connect._model_configuration_source_for_harness",
        lambda harness: {"kind": "gateway", "label": "AI Gateway"},
    )
