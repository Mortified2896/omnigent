      ...
      {evaluation.unresolved_issues && evaluation.unresolved_issues.length > 0 && (
        <div className="mt-1 text-rose-700">
          ...
          <ul className="list-disc pl-4">
            {evaluation.unresolved_issues.map((u, i) => (
              <li key={i}>{u}</li>
            ))}
          </ul>
        </div>
        ...
        <div className="flex gap-1">
          {[1, 2, 3, 4, 5].map((n) => (
            <button
              ...
              className={`flex h-7 w-7 items-center justify-center rounded-md border text-xs ${qualityScore === n ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200 text-slate-700"}`}
            >
              {n}
            </button>
          ))}
...
// Keep REVIEW_VERDICTS referenced so future additions to the
// vocabulary surface here too. (Lint suppression for the unused
// import is unnecessary — the constant is exported and consumed
// by tests.)
export const _REVIEW_VERDICTS_REFERENCE = REVIEW_VERDICTS;
