/** Controlled replacement settings section; not yet registered in the live composer.
    ...
    <section className="advisor-providers" aria-label="Model advisor settings">
      ...
      <fieldset disabled={busy}>
        ...
        {PROVIDER_GROUPS.map((provider) => (
          <ProviderCard key={provider} provider={provider} value={value} options={options}
            idPrefix={idPrefix} onChange={onChange} />
        ))}
        <fieldset className="advisor-own-model">
          ...
          <select id={`${idPrefix}-advisor`} value={value.advisor_choice_id ?? ""}
            onChange={(event) => onChange({ ...value, advisor_choice_id: event.currentTarget.value || null })}>
            <option value="">Choose advisor model + reasoning…</option>
            {value.advisor_choice_id && !knownIds.has(value.advisor_choice_id) ? (
              <option value={value.advisor_choice_id} disabled>Saved advisor unavailable — choose explicitly</option>
            ) : null}
            {PROVIDER_GROUPS.map((provider) => (
              <optgroup key={provider} label={PROVIDER_LABELS[provider]}>
                {groupModels(options, provider).flatMap((model) => model.options.map((option) => (
                  <option key={option.choice_id} value={option.choice_id} disabled={!option.available}>
        ...
        <label htmlFor={`${idPrefix}-balance`}>
          Decision balance: {value.human_probability_percent}% me / {100 - value.human_probability_percent}% advisor
        </label>
        <input id={`${idPrefix}-balance`} className="advisor-balance" type="range" min={0} max={100} step={5}
          value={value.human_probability_percent}
          onChange={(event) => onChange({ ...value, human_probability_percent: Number(event.currentTarget.value) })} />
            ...
            {value.unresolved_legacy_ids.map((id) => (
              <div key={id} className="advisor-missing">
                ...
                <button type="button" onClick={() => onChange({
                  ...value, unresolved_legacy_ids: value.unresolved_legacy_ids.filter((key) => key !== id),
                })}>Remove saved reference</button>
              </div>
            ))}
          </fieldset>
        ) : null}
        <footer>
          ...
          <button type="button" disabled={!props.dirty} onClick={props.onSave}>Save defaults</button>
          <p>{props.dirty ? "Unsaved changes apply only to this round until saved." : "Defaults are saved for new chats."}</p>
        </footer>
      </fieldset>
...
function ProviderCard({ provider, value, options, idPrefix, onChange }: ProviderCardProps) {
  ...
  const visibleIds = new Set(models.flatMap((model) => model.options.map((option) => option.choice_id)));
  ...
  return (
    <section className="advisor-provider-card" aria-labelledby={titleId}>
      <div className="advisor-provider-heading">
        <button id={titleId} className="advisor-disclosure" type="button"
          aria-expanded={!selected.collapsed} aria-controls={panelId}
          onClick={() => onChange(updateProvider(value, provider, { collapsed: !selected.collapsed }))}>
        ...
        <label className="advisor-switch" title="Pause answers without clearing selections">
          <input type="checkbox" role="switch" checked={selected.enabled}
            aria-label={`Enable ${provider === "openai" ? "OpenAI" : "GLM"} answers`}
            onChange={(event) => onChange(toggleProvider(value, provider, event.currentTarget.checked))} />
      ...
      <div id={panelId} hidden={selected.collapsed}>
        <fieldset className="advisor-transport">
          ...
          <label>
            <input type="radio" name={`${idPrefix}-${provider}-transport`}
          ...
          <p>The advisor chooses the model, not the connection. Fallback keeps the same model, reasoning and plan.</p>
          {value.route_review_required.includes(provider) ? (
            <div>
              ...
              <button type="button" onClick={() => onChange(selectTransport(
                value, provider, selected.transport_preference,
              ))}>Confirm connection preference</button>
            </div>
          ) : null}
        </fieldset>
        {!selected.enabled ? <p className="advisor-paused">Answers paused. Your checked levels stay saved.</p> : null}
        {models.length === 0 ? <p>No qualified models in this provider's current catalog.</p> : null}
        {models.map((model) => (
          <fieldset key={model.model_id} className="advisor-model-row">
            ...
            <div className="advisor-efforts">
              {model.options.map((option) => {
                ...
                return (
                  <label key={option.choice_id} className={`advisor-effort${checked ? " is-selected" : ""}`}
                    title={!option.available ? option.unavailable_reason ?? "Unavailable" : undefined}>
                    <input type="checkbox" checked={checked} disabled={!option.available && !checked}
                      aria-label={`${model.display_name}: ${effortLabel(option.reasoning_effort)}`}
                      onChange={(event) => onChange(toggleEffort(value, provider, option.choice_id, event.currentTarget.checked))} />
                    {effortLabel(option.reasoning_effort)}
                    {!option.available ? <span className="advisor-unavailable-label">unavailable</span> : null}
                  </label>
                );
              })}
            </div>
        ...
        {missing.map((id) => (
          <label key={id} className="advisor-missing">
            <input type="checkbox" checked onChange={() => onChange(toggleEffort(value, provider, id, false))} />
