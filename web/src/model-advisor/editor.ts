...
export function samePreferences(a: AdvisorPreferences, b: AdvisorPreferences): boolean {
  return a.enabled === b.enabled && a.advisor_candidate_id === b.advisor_candidate_id &&
...
export function editorReducer(state: EditorState, action: EditorAction): EditorState {
  if (action.type === "scope") return action.scope === state.scope ? state : initialEditor(action.scope);
  if (action.type === "edit") {
    if (!state.draft) return state; // No edits/writes before initial hydration.
    return { ...state, draft: action.preferences, dirty: !state.saved ||
      !samePreferences(state.saved.preferences, action.preferences), error: null };
  }
  ...
  if (action.type === "hydrate") {
    ...
    return { ...state, saved: action.saved, draft: action.saved.preferences, dirty: false, error: null };
  }
  ...
  return { ...state, saved: action.saved,
    ...
    dirty: !unchanged, error: null };
}

export function toggleCandidate(prefs: AdvisorPreferences, id: string, included: boolean): AdvisorPreferences {
  const choices = new Set(prefs.allowed_candidate_ids);
  if (included) choices.add(id); else choices.delete(id);
  return { ...prefs, allowed_candidate_ids: [...choices] };
}

export function optionLabel(option: AdvisorOption): string {
  const lane = option.lane_id === "codex-direct" ? "ChatGPT plan · Direct" :
    option.lane_id === "glm-direct" ? "Z.AI Direct" : "OmniRoute GLM";
  const effort = option.reasoning_effort === "not_applicable" ? "No effort setting" : `${option.reasoning_effort} reasoning`;
  return `${option.display_name} · ${lane} · ${effort}`;
}

export function validateEditor(prefs: AdvisorPreferences, options: readonly AdvisorOption[]): string | null {
  if (!Number.isInteger(prefs.human_probability_percent) || prefs.human_probability_percent < 0 || prefs.human_probability_percent > 100)
  ...
  for (const id of prefs.allowed_candidate_ids) {
    if (!byId.get(id)?.available) return "A saved answer choice is unavailable. Remove it or restore its connection.";
  }
