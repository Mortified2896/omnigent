...
// Bulk archive/delete must be conservative when the row lacks `owner` but does
// carry an effective permission level: treat read/edit/manage as unavailable
// rather than assuming the viewer owns the session.
function canBulkMutateConversation(conversation: Conversation, viewerId: string | null): boolean {
  const owner = conversation.owner ?? null;
  if (owner !== null) return owner === viewerId;
  return isOwnerLevel(conversation.permission_level ?? null);
}
...
function BulkActionBar({
  ...
  const ownedSelected = useMemo(
    () => selectedConversations.filter((c) => canBulkMutateConversation(c, viewerId)),
