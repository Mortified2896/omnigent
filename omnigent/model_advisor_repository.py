...
from sqlalchemy import Column, Engine, Integer, MetaData, String, Table, Text, insert, select, update
...
metadata = MetaData()
records = Table(
    "model_advisor_records", metadata,
...
class AdvisorRepository:
    ...
    def _read(self, connection: Connection, key: str) -> Record | None:
        ...
        row = connection.execute(select(records).where(records.c.scope_key == key)).mappings().first()
        return None if row is None else Record(row["version"], row["state"], json.loads(row["payload"]))
    ...
    def _cas(self, connection: Connection, key: str, previous: Record, state: str, payload: dict) -> Record:
        ...
        if result.rowcount != 1:
            raise AdvisorConflict("Concurrent update; reload the existing record, do not retry execution")
    ...
    def save_preferences(
        self, owner: str, host: str, preferences: AdvisorPreferences,
        *, expected_version: int, profile: str = "default",
    ) -> Record:
        ...
        try:
            with self.engine.begin() as connection:
                ...
                if previous is None:
                    ...
                    connection.execute(insert(records).values(scope_key=key, version=1, state="saved", payload=canonical_json(payload)))
    ...
    def reserve_round(self, frozen: FrozenRound) -> Claim:
        ...
        try:
            with self.engine.begin() as connection:
                connection.execute(insert(records).values(scope_key=key, version=1, state="advisor_pending", payload=canonical_json(payload)))
            ...
            if previous.payload["fingerprint"] != frozen.fingerprint:
                raise AdvisorConflict("Round ID reused with a different prompt, pool or settings")
    ...
    def finish_advice(
        self, owner: str, host: str, round_id: str, raw_advice: str,
        *, randbelow: Callable[[int], int],
        ...
        with self.engine.begin() as connection:
            ...
            payload = {**previous.payload, "advice_digest": advice_digest, "review": review.to_payload()}
            result = self._cas(connection, key, locked, "awaiting_confirmation", payload)
            return Claim(result, True)

    def confirm(
        self, owner: str, host: str, round_id: str, *, expected_version: int,
        live_catalog: PoolSnapshot, override_candidate_id: str | None = None,
        ...
        with self.engine.begin() as connection:
            ...
            final = confirm_review(frozen, review, live_catalog, override_candidate_id=override_candidate_id, reason=reason)
            payload = {
                **previous.payload, "review": final.to_payload(), "confirmation": confirmation,
    ...
    def cancel(self, owner: str, host: str, round_id: str, *, expected_version: int) -> Record:
        ...
        with self.engine.begin() as connection:
            previous = self._required(connection, key)
            if previous.version != expected_version or previous.state not in {"advisor_pending", "awaiting_confirmation"}:
