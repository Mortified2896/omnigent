...
TRUSTED_ROOT_INSTRUCTION = (
    ...
    "is required. Never self-upgrade. Follow the current HomeLab topology: "
    "single-primary releases need an external controller; peer supervision "
    "applies only to deliberately provisioned peers. Never reactivate retired "
    "O2 to satisfy a legacy runbook. Do not treat required root access as a blocker."
)
    ...
    *,
    ...
    return (
        append_framework_instructions(
            base_instructions,
            [*_framework_instructions_for(spec), *effective_framework_instructions],
        )
