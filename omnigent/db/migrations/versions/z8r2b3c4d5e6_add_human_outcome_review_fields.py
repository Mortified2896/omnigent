...
revision = "z8r2b3c4d5e6"
down_revision = "z7r2b3c4d5e6"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.add_column(
        "task_reviews",
        sa.Column("learning_eligible", sa.Boolean(), nullable=False, server_default="0"),
    )
    ...
    op.add_column(
        "task_reviews", sa.Column("preferred_reasoning_effort", sa.String(16), nullable=True)
    )
    op.add_column("task_reviews", sa.Column("source_evaluation_id", sa.String(64), nullable=True))
    op.add_column(
        "task_reviews",
        sa.Column("review_schema_version", sa.SmallInteger(), nullable=False, server_default="1"),
    )
    ...
    with op.batch_alter_table("task_reviews") as batch:
        ...
        batch.create_check_constraint(
            "ck_task_reviews_learning",
            "learning_eligible = false OR review_action IN ('accepted','adjusted')",
        )
        batch.create_check_constraint(
            "ck_task_reviews_route_fit",
            "route_fit IS NULL OR route_fit IN "
            "('appropriate','too_weak','overkill','wrong_capability','unsure')",
        )
        batch.create_check_constraint(
            "ck_task_reviews_failure_attribution",
            "failure_attribution IS NULL OR failure_attribution IN "
            "('router','model','harness','environment','permissions',"
            "'task_definition','external_service','unknown')",
        )
