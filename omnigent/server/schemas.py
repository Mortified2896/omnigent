...
class SessionForkRequest(BaseModel):
    ...
    :param codex_bypass_sandbox: Opt-in for the DANGEROUS codex-native
        ...
        stamps ``omnigent.harnesses.codex_native.main.bypass_sandbox`` on the fork; ``False``
...
class SessionCollaborationModeEvent(_SSEEventBase):
    ...
    Category: **transient** (SSE-only). The server also writes
    ``omnigent.harnesses.codex_native.main.collaboration_mode`` on the conversation labels,
...
class SessionPermissionModeEvent(_SSEEventBase):
    ...
    Category: **transient** (SSE-only). The server also writes
    ``omnigent.harnesses.claude_native.main.permission_mode`` on the conversation labels, so
...
class SessionCodexApprovalModeEvent(_SSEEventBase):
    ...
    Category: **transient** (SSE-only). The server also writes
    ``omnigent.harnesses.codex_native.main.approval_mode`` on the conversation labels (and
