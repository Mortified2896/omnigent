...
- Reads: ``GET /v1/sessions/{id}/routing-turns``,
  ``GET /v1/sessions/{id}/task-runs``,
...
def _serialise_run(run: TaskRun) -> dict[str, Any]:
    ...
    return {
        ...
        "routing_proposal_id": run.routing_proposal_id,
        "routing_decision_id": run.routing_decision_id,
...
def _routing_proposal_payload(turn: RoutingTurnAudit) -> dict[str, Any]:
    proposal = turn.proposal
    try:
        payload = json.loads(proposal.proposal_payload_excerpt)
    except json.JSONDecodeError:
        payload = {}
    if not isinstance(payload, dict):
        payload = {}
    profile = get_route_profile(proposal.original_route_id)
    payload.update(
        {
            "routing_proposal_id": proposal.id,
            "recommended_harness": proposal.original_harness,
            "omniroute_route_id": proposal.original_route_id,
            "reasoning_effort": proposal.original_reasoning_effort,
            "permission_mode": proposal.original_permission_mode,
            "omniroute_requires_explicit_approval": proposal.requires_explicit_approval,
            "router_evaluator_route": proposal.evaluator_route_id,
            "actual_evaluator_provider": proposal.evaluator_provider,
            "actual_evaluator_model": proposal.evaluator_model,
...
def _serialise_routing_turn(turn: RoutingTurnAudit) -> dict[str, Any]:
    proposal = turn.proposal
    ...
    final_selection = (
        {
            "recommended_harness": decision.final_harness,
            "provider": decision.final_provider,
            "model": decision.final_model,
            "omniroute_route_id": decision.final_route_id,
    ...
    frontend_action = (
        "accept"
        if decision is not None and decision.action in {"approved", "changed"}
        else "decline"
        if decision is not None and decision.action == "declined"
        else None
    )
    return {
        "id": proposal.id,
        "elicitation_id": proposal.elicitation_id,
        "status": "responded" if decision is not None else "pending",
        "proposal": _routing_proposal_payload(turn),
        "action": frontend_action,
        "durable_action": decision.action if decision is not None else None,
        "response": (
            {"action": frontend_action, "content": {"final_selection": final_selection}}
        ...
        "evaluator_provenance": {
            "route_id": proposal.evaluator_route_id,
            "provider": proposal.evaluator_provider,
            "model": proposal.evaluator_model,
            "billing_class": proposal.evaluator_billing_class,
            "fallback_used": proposal.evaluator_fallback_used,
            "decision_id": proposal.evaluator_decision_id,
            "selection_strategy": proposal.evaluator_selection_strategy,
        },
        "triggering_message_id": run.triggering_message_id if run is not None else None,
        "response_id": run.response_id if run is not None else None,
        "created_at": proposal.created_at,
        "decided_at": decision.created_at if decision is not None else None,
    }
...
def create_task_outcomes_router(
    ...
    # ── GET /sessions/{id}/routing-turns ───────────────────────────

    @router.get("/sessions/{session_id}/routing-turns")
    async def list_session_routing_turns(
        request: Request,
        session_id: str,
        ...
        turns = await _call_store(
            store.list_routing_turns_for_conversation,
            conversation_id=session_id,
            limit=limit,
        )
        return {"object": "list", "turns": [_serialise_routing_turn(turn) for turn in turns]}
