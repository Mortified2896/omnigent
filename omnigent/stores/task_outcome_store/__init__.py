...
class RoutingDecisionConflictError(ValueError):
    """A proposal already has a different immutable decision."""


@dataclasses.dataclass(frozen=True)
class RoutingTurnAudit:
    """Durable proposal, optional decision, and linked execution run."""
...
@dataclasses.dataclass(frozen=True)
class CreateRoutingProposalInput:
    conversation_id: str
    elicitation_id: str
    user_message: str
...
@dataclasses.dataclass(frozen=True)
class CreateRoutingDecisionInput:
    proposal_id: str
    action: str
...
class TaskOutcomeStore(ABC):
    ...
    def create_routing_proposal(self, data: CreateRoutingProposalInput) -> RoutingProposal:
        """Persist one immutable proposal before asking for approval."""
