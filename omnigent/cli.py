...
def server(
    ...
    # Task-outcome tracking: mirrors the comment-store pattern (one
    # SQLAlchemy store per process, opened against the same DB URI
    # so it shares the engine pool + transactions with the other
    # stores). Disabled when ``OMNIGENT_TASK_OUTCOMES_ENABLED=0``
    # so operators can opt out without a code change.
